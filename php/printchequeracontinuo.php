...
//- Obtiene ID de cheques a imprimir
$idbanco = (int)$_GET['idbanco'];
$ndel = (int)$_GET['c_del'];
$nal = (int)$_GET['c_al'];
$query ="SELECT id FROM tranban WHERE numero >= $ndel AND numero <= $nal AND tipotrans = 'C' AND idbanco = $idbanco AND operado = 0 AND anulado = 0" ;
...
for($i = 0; $i < $registros; $i++){
    ...
    */
    ...
    $pdf->Cell(1 * $conv);
    $pdf->Cell(8.5 * $conv, 0.25 * $conv, 'Guatemala, '.$cheque->dia.' de '.$meses[(int)$cheque->mes].' de '.$cheque->anio, $borde, 0);
    $pdf->Cell(2 * $conv);
    $pdf->Cell(4.5 * $conv, 0.25 * $conv, $cheque->montostr, $borde, 0);
    $pdf->Ln();
    $pdf->Cell(1 * $conv);
    $pdf->Cell(12.3 * $conv, 0.8 * $conv, iconv('UTF-8', 'windows-1252', $cheque->beneficiario), $borde, 0);
    $pdf->Ln();
    $pdf->Cell(1 * $conv);
    $pdf->Cell(12.5 * $conv, 0.8 * $conv, $n2l->to_word_int($cheque->monto), $borde, 0);
    $pdf->Ln(2.5 * $conv);
