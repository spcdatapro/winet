...
$app->post('/rptdescuadres', function(){
    ...
    $query.= "LEFT JOIN (SELECT origen, idorigen, SUM(debe) AS totdebe, SUM(haber) AS tothaber FROM detallecontable WHERE origen = 1 GROUP BY origen, idorigen HAVING totdebe <> tothaber) z ON z.idorigen = b.id ";
    $query.= "WHERE b.fecha >= '".$d->fdelstr."' AND b.fecha <= '".$d->falstr."' AND c.idempresa = ".$d->idempresa." AND (z.idorigen = NULL OR z.totdebe <> z.tothaber) ";
    //#Compras -> origen = 2
    $query.= "UNION ALL ";
    ...
    $query.= "LEFT JOIN (SELECT origen, idorigen, SUM(debe) AS totdebe, SUM(haber) AS tothaber FROM detallecontable WHERE origen = 2 GROUP BY origen, idorigen HAVING totdebe <> tothaber) z ON z.idorigen = b.id ";
    $query.= "WHERE b.idreembolso = 0 AND b.fechaingreso >= '".$d->fdelstr."' AND b.fechaingreso <= '".$d->falstr."' AND b.idempresa = ".$d->idempresa." AND (z.idorigen = NULL OR z.totdebe <> z.tothaber) ";
    //#Ventas -> origen = 3
    $query.= "UNION ALL ";
    ...
    $query.= "LEFT JOIN (SELECT origen, idorigen, SUM(debe) AS totdebe, SUM(haber) AS tothaber FROM detallecontable WHERE origen = 4 GROUP BY origen, idorigen HAVING totdebe <> tothaber) z ON z.idorigen = b.id ";
    $query.= "WHERE b.fecha >= '".$d->fdelstr."' AND b.fecha <= '".$d->falstr."' AND b.idempresa = ".$d->idempresa." AND (z.idorigen = NULL OR z.totdebe <> z.tothaber) ";
    //#Reembolsos -> origen = 5
    $query.= "UNION ALL ";
    /*
    ...
    */
    ...
    $query.= "CONCAT('Ree/CC No. ', d.id, ' ', b.serie, '-', b.documento, ' ') AS referencia, b.conceptomayor AS concepto, b.id, 5 AS origen ";
    ...
    */
    ...
    $query.= "LEFT JOIN (SELECT origen, idorigen, SUM(debe) AS totdebe, SUM(haber) AS tothaber FROM detallecontable WHERE origen = 7 GROUP BY origen, idorigen HAVING totdebe <> tothaber) z ON z.idorigen = b.id ";
    $query.= "WHERE b.fecha >= '$d->fdelstr' AND b.fecha <= '$d->falstr' AND b.idempresa = $d->idempresa AND (z.idorigen = NULL OR z.totdebe <> z.tothaber) ";
    ...
    $query.= "FROM tranban b INNER JOIN banco c ON c.id = b.idbanco INNER JOIN tipomovtranban d ON d.abreviatura = b.tipotrans ";
    $query.= "LEFT JOIN (SELECT origen, idorigen, SUM(debe) AS totdebe, SUM(haber) AS tothaber FROM detallecontable WHERE origen = 9 GROUP BY origen, idorigen HAVING totdebe <> tothaber) z ON z.idorigen = b.id ";
    $query.= "WHERE b.fechaliquida >= '$d->fdelstr' AND b.fechaliquida <= '$d->falstr' AND c.idempresa = $d->idempresa AND (z.idorigen = NULL OR z.totdebe <> z.tothaber) ";
    $query.= "ORDER BY 2, 1";
